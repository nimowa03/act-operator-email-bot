...
- 템플릿 이름: Email Assistant (slug: email-assistant, snake: email_assistant)
...
```json
{
  ...
  "graphs": {
    ...
    "email_bot": "./casts/email_bot/graph.py:email_bot_graph"
  },
  "env": ".env"
}
```
...
- 각 Cast는 `casts/email_bot/modules/state.py`에서 세 가지 구분된 상태 스키마를 사용합니다:
...
| 스킬 | 목적 | 사용 시점 |
|------|------|----------|
| `@architecting-act` | 아키텍처 설계 | Email Assistant 프로젝트 시작 시 계획, 새 cast 계획, 구조 불명확, CLAUDE.md 필요 시 (개발 명령어 포함) |
...
`Node` (동기) 및 `AsyncNode` (비동기)의 실제 예제는 `casts/email_bot/modules/nodes.py`를 참조하세요.
